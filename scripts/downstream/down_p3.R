...
#------------------------------------------------------------------------------#
#### Change name ####
path <- here::here("results","down", "down_p3")
...
numrem <- c(1,4,8,16) #segments where removal occurs
...
for(r in 1:Rem){
  site.traps[1,1:numrem[r],1:P, 1:S,r]<- tail(which(initpop[1:22] > 0), numrem[r])
}
