...
##### plot 2 #####
all_Ntotal2 <- all_Ntotal %>% filter(rem != '0')
...
all_Ntotal2$rem2 <- all_Ntotal2$rem
all_Ntotal2$rem2[all_Ntotal2$rem2 == '1'] <- '4'
...
##### plot 3 #####
all_Ntotal3 <- all_Ntotal %>% filter(rem != '0' & rem != '1')

all_Ntotal_rem1 <- all_Ntotal %>% filter(rem == '1')

all_Ntotal_rem1$location <- 'down1'

all_Ntotal3 <- rbind(all_Ntotal3, all_Ntotal_rem1)

level_order3 <- c("abund", "down1", "down", "edge", "grow", "random")
...
all_Ntotal3$rem2 <- all_Ntotal3$rem
all_Ntotal3$rem2[all_Ntotal3$rem2 == '1'] <- '4'

all_Ntotal3 %>% 
  ggplot(aes(x = factor(location, level = level_order3), y = count, 
             group = interaction(rem, location)))+
  geom_boxplot(aes( fill = as.factor(rem)), position="dodge",alpha = 0.5)+
  geom_hline(yintercept = nc.val, linetype = 2) + 
...
##### plot 2 #####
all_Ninvade2 <- all_Ninvade %>% filter(rem != '0')
...
all_Ninvade2$rem2 <- all_Ninvade2$rem
all_Ninvade2$rem2[all_Ninvade2$rem2 == '1'] <- '4'
