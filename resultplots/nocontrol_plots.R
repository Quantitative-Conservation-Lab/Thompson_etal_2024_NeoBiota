...
#### Minimize colombia ####
# N_colombia <- N_allsims  %>% filter(primary == max(N_allsims$primary) & segment == 22)
# 
# ggplot(N_colombia, aes(x=param, y=count)) + 
#   geom_boxplot(aes(group = param))
# 
# ggplot(N_colombia, aes(x=primary, y=count)) + 
#   geom_boxplot(aes(group = primary))
# 
# summary(N_colombia$count)
