...
path2 <- "abund_truth"
file_name = paste(path, path2,'DR_abund_N_all.csv',sep = '/')
N_allsegssims_abund1 <- fread(file_name)
N_allsegssims_abund1 <- data.frame(N_allsegssims_abund1)[-1]

#Dafter
file_name = paste(path, path2,'DR_abund_D_all.csv',sep = '/')
Dafter_allsegssims_abund1 <- fread(file_name)
Dafter_allsegssims_abund1 <- data.frame(Dafter_allsegssims_abund1)[-1]

#Initial pop: 0
Ninit_allsegssims_abund1 <- filter(N_allsegssims_abund1, primary == 1)
Ninit_allsegssims_abund1$primary <- 0

#remove age = 0
Ninit_allsegssims_abund1$age <- Ninit_allsegssims_abund1$age -1
Ninit_allsegssims_abund1 <- filter(Ninit_allsegssims_abund1, age > 0)

Dafter_allsegssims_abund1$age <- Dafter_allsegssims_abund1$age -1
Dafter_allsegssims_abund1 <- filter(Dafter_allsegssims_abund1, age > 0)

#Combine init with Dafter
Dafter_mean_allseg_abund1 <- rbind(Ninit_allsegssims_abund1, Dafter_allsegssims_abund1)

N_allsims <- Dafter_mean_allseg_abund1

N_abund1 <- aggregate(count ~ primary + param + sim + segment, 
                         data = as.data.frame(N_allsims),
                         FUN = sum)



N_abund2 <- aggregate(count ~ primary + param + segment, 
                         data = as.data.frame(N_abund1),
                         FUN = mean)

N_abund <- aggregate(count ~ primary + segment, 
                          data = as.data.frame(N_abund2),
                          FUN = mean)
...
finalN_abund <- N_abund %>% filter(primary == max(primary))
finalN_abund <- finalN_abund[-1]
colnames(finalN_abund)[1:2] <- c('Segment', 'Final')

jdr_abund <- merge(jdr_20k_sf, finalN_abund, by  = "Segment")

initN_abund <- N_abund %>% filter(primary == 0)
initN_abund <- initN_abund[-1]
colnames(initN_abund)[1:2] <- c('Segment', 'Initial')

jdr_abund <- merge(jdr_abund, initN_abund, by = "Segment")

jdr_abund <- st_transform(jdr_abund, "WGS84")

#Custom color palettes that depends on final pop
pal <- colorBin("YlOrRd", domain = jdr_abund$Final, bins = 6)

colors <- c("black", "#313695", "#74ADD1","gold", "darkorange", "red", "red4" )
pal <- colorBin(colors, domain = jdr_abund$Final, bins = 6)

leaflet() %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% # Add a nice, neutral background
  #addProviderTiles(providers$Esri.NatGeoWorldMap) %>% 
  addPolygons(data = jdr_abund, # Add polygons 
              color = ~pal(jdr_abund$Final), # Color the segment polygons
...
colnames(jdr_abund)[5] <- "Final Crayfish Abundance (per segment)"
...
tmap_options(bg.color = 'white', legend.text.color = 'black') +
  ...
  tm_layout(legend.position = c("right", "top"), legend.text.size = 1, 
            legend.width = 2, frame = color,frame.lwd = 12)
