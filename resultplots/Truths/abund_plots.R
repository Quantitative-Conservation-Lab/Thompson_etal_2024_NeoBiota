...
N_noabunds  <- aggregate(pop ~ sim + param, 
                         data = as.data.frame(N_lastsegs), 
                         FUN = sum) #sum across ages

ggplot(N_noabunds, aes(x=param, y=pop)) + 
  geom_boxplot(aes(group = param))

N_noabunds$bar <- 1

ggplot(N_noabunds, aes(x=bar, y=pop/35)) + 
  geom_boxplot(aes(group = bar))

(summary(N_noabunds$pop)/35)

file_name = paste(path,path2, 'DR_abund_minspatial.csv',sep = '/')
write.csv(N_noabunds,file_name)
