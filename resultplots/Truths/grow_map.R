...
N_grow1 <- aggregate(count ~ primary + param + sim + segment, 
                         data = as.data.frame(N_allsims),
                         FUN = sum)



N_grow2 <- aggregate(count ~ primary + param + segment, 
                         data = as.data.frame(N_grow1),
                         FUN = mean)

N_grow <- aggregate(count ~ primary + segment, 
                          data = as.data.frame(N_grow2),
                          FUN = mean)
...
finalN_grow <- N_grow %>% filter(primary == max(primary))
finalN_grow <- finalN_grow[-1]
colnames(finalN_grow)[1:2] <- c('Segment', 'Final')

jdr_grow <- merge(jdr_20k_sf, finalN_grow, by  = "Segment")

initN_grow <- N_grow %>% filter(primary == 0)
initN_grow <- initN_grow[-1]
colnames(initN_grow)[1:2] <- c('Segment', 'Initial')

jdr_grow <- merge(jdr_grow, initN_grow, by = "Segment")

jdr_grow <- st_transform(jdr_grow, "WGS84")

#Custom color palettes that depends on final pop
pal <- colorBin("YlOrRd", domain = jdr_grow$Final, bins = 6)

colors <- c("black", "#313695", "#74ADD1","gold", "darkorange", "red", "red4" )
pal <- colorBin(colors, domain = jdr_grow$Final, bins = 6)

leaflet() %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% # Add a nice, neutral background
  #addProviderTiles(providers$Esri.NatGeoWorldMap) %>% 
  addPolygons(data = jdr_grow, # Add polygons 
              color = ~pal(jdr_grow$Final), # Color the segment polygons
...
colnames(jdr_grow)[5] <- "Final Crayfish Abundance (per segment)"

breaks = c(0.00001, 1, 2, 4, 6, 8, 10) * 100000

colors <- c("grey", "lightblue", "gold", "darkorange", "red", "black" )

color <- RColorBrewer::brewer.pal(11, "Paired")[9]
...
colnames(N_grow)[1:3] <- c("Month", "Segment", "Abundance")

jdr_grow <- merge(jdr_20k_sf, N_grow, by  = "Segment")
...
tmap_animation(
  jdr_animation , filename = "jdr_grow.gif",
  delay = 20)
